...
# Функция: Запуск ASF Bot в установленном дистрибутиве
run_asf_bot() {
    check_distro_installed

    # Если установлен Ubuntu, запускаем команду для Ubuntu
    if [ "$ubuntu_installed" = true ]; then
        echo "Запуск ASF Bot на Ubuntu..."
        proot-distro login ubuntu -- python3 bot.py
    # Если установлен Debian, запускаем команду для Debian
    elif [ "$debian_installed" = true ]; then
        echo "Запуск ASF Bot на Debian..."
        proot-distro login debian -- python3 bot.py
    else
